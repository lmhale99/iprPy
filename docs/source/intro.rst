...
Scientific research workprocess
===============================
...
- Do the original conclusions hold true if 
...
- Python :any:`calculation scripts <tutorial/calculation>` are used to collect
...
- All of a calculation script’s variable parameters are read in through a
  simple :any:`input parameter <tutorial/inputfile>` file. This highlights the
...
- Upon successful completion, the calculation scripts produce :any:`XML- or
  JSON-formatted results records <tutorial/recordformat>`. Records in these
...
- :any:`Jupyter demonstration Notebooks <tutorial/jupyter>` are provided for
  each calculation.
- The high-throughput tools can be directly executed from stand-alone
  ...
  commands <tutorial/highthroughput/inline>`.
- New :any:`calculation, record, and database styles 
  <tutorial/highthroughput/classes>` can be easily added in a modular fashion.
