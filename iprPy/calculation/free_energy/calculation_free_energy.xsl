...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="calculation-free-energy">
    <div>
      
      <xsl:variable name="calckey" select="key"/>

      <h2>Free energy crystal calculation results</h2>

      <ul>
        <li><b><xsl:text>UUID4: </xsl:text></b><xsl:value-of select="key"/></li>
        <li><b><xsl:text>Calculation: </xsl:text></b><a href="https://www.ctcms.nist.gov/potentials/iprPy/notebook/free_energy.html">free_energy</a></li>
      ...
      <xsl:if test="phase-state">
        ...
        <ul>
          ...
          <li>
            <b><xsl:text>Spring constant estimate run length: </xsl:text></b>
            <xsl:value-of select="calculation/run-parameter/springsteps"/>
          </li>
          <li>
      ...
      <xsl:if test="volume">
        ...
        <ul>
          ...
          <li>
            <b><xsl:text>Einstein solid spring constants (eV/Angstrom^2): </xsl:text></b>
            <xsl:for-each select="spring-constants/value">
          ...
          <li>
            <b><xsl:text>Helmholtz free energy of reference Einstein solid (eV/atom): </xsl:text></b>
