...
from .energy_check_ase import energy_check_ase
from ...calculation_subset import (LammpsPotential, LammpsCommands, Units,
                                   AtommanSystemLoad)
from ...input import boolean

class EnergyCheckASE(Calculation):
...
############################# Core properties #################################
    ...
    def __init__(self,
        ...
        **kwargs : any
        ...
        self.__pressure_xy = None
        self.__pressure_xz = None
        self.__pressure_yz = None
        self.__forces = None

        # Define calc shortcut
        self.calc = energy_check_ase
...
############################## Class attributes ###############################
    ...
    def pressure_xy(self) -> float:
        """float: The measured xy component of pressure for the system"""
        if self.__pressure_xy is None:
            raise ValueError('No results yet!')
        return self.__pressure_xy
    
    @property
    def pressure_xz(self) -> float:
        """float: The measured xz component of pressure for the system"""
        if self.__pressure_xz is None:
            raise ValueError('No results yet!')
        return self.__pressure_xz
    
    @property
    def pressure_yz(self) -> float:
        """float: The measured yz component of pressure for the system"""
        if self.__pressure_yz is None:
            raise ValueError('No results yet!')
        return self.__pressure_yz
    
    @property
    def forces(self) -> np.ndarray:
        """float: The measured atomic forces for the system"""
...
########################### Data model interactions ###########################
    ...
    def build_model(self) -> DM:
        ...
        if self.status == 'finished':
            ...
            mps['pressure-zz'] = uc.model(self.pressure_zz,
                                          self.units.pressure_unit)
            mps['pressure-xy'] = uc.model(self.pressure_xy,
                                          self.units.pressure_unit)
            mps['pressure-xz'] = uc.model(self.pressure_xz,
                                          self.units.pressure_unit)
            mps['pressure-yz'] = uc.model(self.pressure_yz,
                                          self.units.pressure_unit)
    ...
    def load_model(self,
        ...
        if self.status == 'finished':
            ...
            self.__pressure_xy = uc.value_unit(mps['pressure-xy'])
            self.__pressure_xz = uc.value_unit(mps['pressure-xz'])
            self.__pressure_yz = uc.value_unit(mps['pressure-yz'])
...
########################### Calculation interactions ##########################
    ...
    def process_results(self, results_dict: dict):
        ...
        """
        self.__potential_energy = results_dict['PotEng']
        self.__potential_energy_atom = results_dict['PotEngAtom']
        self.__pressure_xx = results_dict['Pxx']
        self.__pressure_yy = results_dict['Pyy']
        self.__pressure_zz = results_dict['Pzz']
        self.__pressure_xy = results_dict['Pxy']
        self.__pressure_xz = results_dict['Pxz']
        self.__pressure_yz = results_dict['Pyz']
        self.__forces = results_dict.get('F', None)
