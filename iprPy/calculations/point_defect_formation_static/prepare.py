...
def description():
    ...
    return "The point_defect_formation_static calculation constructs a system, inserts a point defect, relaxes atomic positions with an energy/force minimization and computes the formation energy of the defect by comparing to the perfect system.  The high-throughput version uses data models that define parameter sets associated with a particular type of defect."
    ...
    for potential_file, potential_dir in zip(prepare_variables.aslist('potential_file'), 
        ...
        for load, load_options, load_elements, box_parameters in zip(prepare_variables.aslist('load'), 
            ...
            for size_mults in prepare_variables.aslist('size_mults'):                                                     
                ...
                for point_defect_model in prepare_variables.aslist('point_defect_model'):
                    ...
                    calculation_variables['point_defect_model'] = point_defect_model
                    ...
                    point_defect_id = input_dict['point_defect_model']['point-defect-parameters']['point-defect']['id']
                    ...
                    for symbols in atomman_input.yield_symbols(load, load_options, load_elements, global_variables, potential):
                        ...
                        record_dir = os.path.join(calculation_variables['lib_directory'], str(potential), '-'.join(symbols), system_family, __calc_type__, point_defect_id)
                        ...
                        if __is_new_record(record_dir, record):
                            ...
                            if calculation_variables['copy_files']:
                                ...
                                shutil.copy(point_defect_model, sim_dir)
                                ...
                                calculation_variables['point_defect_model'] = os.path.basename(point_defect_model)
...
def __initial_setup(inline_terms, global_variables):
    ...
    assert len(prepare_variables.aslist('point_defect_model')) > 0,                                            'no point_defect_model found'
...
def __is_new_record(record_dir, record):
    ...
    match_keys = [['calculation-point-defect-formation', 'calculation', 'script'],
                  ['calculation-point-defect-formation', 'calculation', 'run-parameter', 'size-multipliers'],
                  ['calculation-point-defect-formation', 'calculation', 'run-parameter', 'energy_tolerance'],
                  ['calculation-point-defect-formation', 'calculation', 'run-parameter', 'force_tolerance'],
                  ['calculation-point-defect-formation', 'calculation', 'run-parameter', 'maximum_iterations'],
                  ['calculation-point-defect-formation', 'calculation', 'run-parameter', 'maximum_evaluations'],
                  ['calculation-point-defect-formation', 'potential', 'id'],
                  ['calculation-point-defect-formation', 'system-info', 'artifact'],
                  ['calculation-point-defect-formation', 'system-info', 'symbols'],
                  ['calculation-point-defect-formation', 'point-defect-parameters', 'point-defect', 'id']]
