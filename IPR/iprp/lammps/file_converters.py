...
#Reads a LAMMPS style dump file and returns a iprp.System. 
#Either 'element' term needed in dump style or atomtypes needs to be supplied. 
def read_dump(fname, atomtypes=None):
            ...
            if len(terms) > 0:
                ...
                if readtime:
                    ...
                    for i in xrange(len(terms)):
                        #check for per-AtomType values
                        if prop_names[i] == 'element':
                            element = value
                        elif prop_names[i] == 'mass':
                            mass = float(value)
                        
                        #save per-Atom values
                        elif prop_names[i] != 'id':
                            try:
                                value = int(terms[i])
                            except:
                                value = float(terms[i])
                            atoms[id].set(prop_names[i], value)
                    
                    #hold per-AtomType values 
                    if elements != None:
                        elements[atoms[id].get('type')] = element
                    if masses != None:
                        masses[atoms[id].get('type')] = mass
                ...
                #read boundary condition values if time to do so
                elif bcount < 3:
                    box[bcount][0] = float(terms[0])
                    box[bcount][1] = float(terms[1])
                    if tbox:
                        box[bcount][2] = float(terms[2])
                    bcount += 1
                
                #if not time to read data, check for header information
                else:
                
                    #identify data entry type headers
                    if terms[0] == 'ITEM:':
                    
                        #identify that it is time to read number of atoms
                        if terms[1] == 'NUMBER':
                            readnatoms = True
                            
                        #identify that it is time to read box information
                        if terms[1] == 'BOX':
                            box = np.zeros((3, 3))
                            
                            #differentiate between orthogonal and trigonal boxes
                            if terms[3] == 'xy':
                                tbox = True
                                xx = 6
                            else:
                            ...
                            #read periodic boundary condition information
                        ...
                        #identify that it is time to read atomic information           
                        elif terms[1] == 'ATOMS':
                            
                            #create list of per-Atom property names
                            prop_names = terms[2:]   
                            idindex = prop_names.index('id')
                            
                            #check for per-AtomType property name in prop_list
                            if 'element' in prop_names and 'type' in prop_names:
                                elements = {}
                            else:
                                elements = None
                            if 'mass' in prop_names:
                                masses = {}
                            else:
                                masses = None
                            
                            #raise error if no elemental information is supplied 
                            if atomtypes == None and elements == None:
                                raise ValueError('No element <-> atom type information found!')
                            
                            readtime = True  
        
    #assign AtomType information
    if atomtypes == None:
        atomtypes = []
        for i in xrange(len(elements)):
            if masses == None:
                atomtypes.append(iprp.AtomType(elements[i+1]))
            else:
                atomtypes.append(iprp.AtomType(elements[i+1], masses[i+1]))
